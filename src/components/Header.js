...
const Header = () => {
    ...
    return (
        <header
            className={`fixed w-full top-0 left-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-customGreen shadow-lg py-2' : 'bg-transparent py-4'
                }`}
        >
            <div className="max-w-7xl mx-auto flex justify-between items-center px-6">
                ...
                <nav className="hidden md:flex space-x-8">
                    ...
                    <Link to="/blog" className="text-white hover:text-gray-300 font-poppins font-bold transition duration-300">Blog</Link>
                </nav>
            ...
            {/* Mobile Menu */}
            {isMenuOpen && (
                <nav className={`md:hidden ${isScrolled ? 'bg-customGreen' : 'bg-transparent'} text-white px-6 py-4 space-y-2 font-poppins`}>
                    <a
                        href="#home"
